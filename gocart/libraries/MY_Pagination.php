...
class MY_Pagination extends CI_Pagination
{
	...
	auther : Rameshwar
	date : 10 april 2014
	desc : for use query string	
	*/
	function create_links_query_string()
	{
	// If our item count or per-page total is zero there is no need to continue.
    if ($this->total_rows == 0 OR $this->per_page == 0)
    {
	...
	if($this->suffix!='' && $this->first_link !== FALSE AND $this->cur_page > ($this->num_links + 1)){
	$first_url = ($this->suffix == '') ? $this->base_url : $this->base_url.$this->prefix.'0'.$this->suffix;
