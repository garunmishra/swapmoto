...
function theme_img($uri, $tag=false,$class=false)
{
	if($tag)
	{
		return '<img src="'.theme_url('assets/img/'.$uri).'" alt="'.$tag.'" class="'.$class.'">';
	}
	else
	{
...
function plugins_js($uri)
{
    return '<script type="text/javascript" src="'. base_url(). 'gocart/plugins/' .$uri. '"></script>';
}

function plugins_css($uri)
{
    return '<link href="'. base_url(). 'gocart/plugins/' .$uri. '" type="text/css" rel="stylesheet"/>';
}
...
//to generate an image tag, set tag to true. you can also put a string in tag to generate the alt tag
function get_img($uri, $tag=false)
{
	$CI =& get_instance();
	//return $CI->config->base_url('gocart/themes/'.$CI->config->item('theme').'/'.$uri);
	if($tag)
	{
		return  '<img src="'. $CI->config->base_url().$uri.'" alt="'.$tag.'">';
	}
	else
	{
