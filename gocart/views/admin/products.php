...
if(!empty($term)):
	$term = json_decode($term);
		if(!empty($term->term) || !empty($term->category_id) || ($term->admin_approve !='')):?>
		...
		<div class="row">
			...
			<div class="span8">
					<fieldset>
                     <?php
					 	$options = array(	
						''	=> lang('select_pro_status'),						  
						 	  '0'	=> lang('pro_unapproved'),
			                  '1'	=> lang('pro_approved'),
			                  '2'	=> lang('pro_blocked')
			                );

						echo form_dropdown('admin_approve', $options, set_value('enabled',isset($_POST['admin_approve'])?$_POST['admin_approve']:''), 'class="span2"');
					?>
                   	<?php
	...
	<table class="table table-striped">
		<thead>
			<tr>
                ...
                <th><?php echo sort_url('admin_approve', 'admin_approve', $order_by, $sort_order, $code, $this->config->item('admin_folder'));?></th>
				<th>
	...
	<?php foreach ($products as $product):?>
			<tr>
                ...
                <?php
						...
						echo form_dropdown('product['.$product->id.'][admin_approve]', $options, set_value('enabled',$product->admin_approve), 'class="span2"');
					?>
