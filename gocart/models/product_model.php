...
Class Product_model extends CI_Model
{
	...
	function products($data=array(), $return_count=false)
	{
		...
		else
		{
			...
			if(!empty($data['term']))
			{
				...
				if(!empty($search->admin_approve)){
					$this->db->where('admin_approve', $search->admin_approve);				
				}
	...
	function get_products($condetion = false, $limit = false, $offset = false, $by=false, $sort=false)
	{
		...
		if ($condetion)
		{
			...
			//$this->db->select('category_products.*, LEAST(IFNULL(NULLIF(saleprice, 0), price), price) as sort_price', false)->from('category_products')->f('products')->where(array('category_id'=>$category_id, 'enabled'=>1,'admin_approve'=>1));
			$this->db->select('*, LEAST(IFNULL(NULLIF(saleprice, 0), price), price) as sort_price', false)->from('products')->where($condetion);
	...
	{
		$this->db->select("p.*, cust.firstname");
		$this->db->join('customers cust','user_id=cust.id');
		$result	= $this->db->get_where('products p', array('p.id'=>$id))->row();
		if(!$result)
		{
			return false;
		}
	...
	public function get_company_list(){
		$this->db->select('id company_id, name company_name ');
		$this->db->where('status',1);
		$this->db->order_by('name','asc');		
		return	$this->db->get('company')->result();
	}
	public function get_model_list($id){
		$this->db->select('id model_id, name model_name ');
		$this->db->where('status',1);
		$this->db->where('parent_id',$id);
		$this->db->order_by('name','asc');		
		return	$this->db->get('model')->result();
	}
	public function get_compnyIdbyname($companyName=false){
		if($companyName!=false){
			$this->db->select('id');
			$this->db->where('name',$companyName);
			 $companyDetail =	$this->db->get('company')->result();
		if(!isset($companyDetail[0]->id)){
			$this->db->insert('company',array('name'=>$companyName));
			return $this->db->insert_id();
		}
		else{
			return $companyDetail[0]->id;
		}
	...
	public function get_modelIdbyname($modelaname=false,$catId){
		if($modelaname!=false){
			...
			$this->db->where('name',$modelaname);
			 $modelDetail =	$this->db->get('model')->result();
		if(!isset($modelDetail[0]->id)){
			$this->db->insert('model',array('name'=>$modelaname,'parent_id'=>$catId));
			return $this->db->insert_id();
		}
		else{
			return $modelDetail[0]->id;
		}
		
	}
	}
	
	function get_my_products($data=array(), $return_count=false)
	{
	...
	public  function get_user_product($id,$costumerid)
	{
		$result	= $this->db->get_where('products', array('id'=>$id,'user_id'=>$costumerid))->row();
		if(!$result)
		{
			return false;
		} else { return true; }
	...
	public function get_similar_product($condetion,$limit=false){
			$this->db->select('*, LEAST(IFNULL(NULLIF(saleprice, 0), price), price) as sort_price', false);
